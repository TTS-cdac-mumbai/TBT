...
#handling two conjecutive  halant 
 $syllabified_sentence=~ s/94D 94D /94D /;

#handling halant after character with nukhta ख़् 	राब् , it should be ख़ राब् 
 
 if($syllabified_sentence=~ m/^(...)93C94D/){   
                                           $temp=$1;
                                    #print 'dollor1= '.$1;           
                               $syllabified_sentence=~ s/93C94D /93C /;
                             }

#handling 91594D 937
$syllabified_sentence=~ s/^91594D 937/91594D937/;
...
sub loadPhoneSet
{
	#my @pHbuf = &readFile("./iitm/etc/Hindi_PhoneSet.txt");
         my @pHbuf = &readFile("./iitm/etc/Marathi_PhoneSet.txt");
	my $SCHWA = ();
            #print "phbuf= @pHbuf  ";
