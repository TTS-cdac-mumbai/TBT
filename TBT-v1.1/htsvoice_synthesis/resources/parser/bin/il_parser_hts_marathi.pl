...
# print "words= @words1\n";
  #     foreach $wd (@words1)
	...
	#print "op is $#modfiedWord :::::: $syllabified_sentence\n";
	 print "syllabified sentence =$syllabified_sentence \n";
       ...
       #   my @syllables1= &transliterate($syllabified_sentence);
         # print "syllabified sentence_trans =@syllables1 \n";
          ...
          # print "\t";
           ...
           $syllabified_sentence=~ s/939 923/93994D 923/g;  ##to cope up with lab file generated at IITM
...
sub syllabify {
	...
	for ( my $i = 0 ; $i < @{ $_[0] } ; $i++ ) {
		...
		elsif ((&hex2dec("${ $_[0] }[$i]") > &hex2dec("914")&& &hex2dec("${ $_[0] }[$i]") < &hex2dec("940")	))
		{ 
