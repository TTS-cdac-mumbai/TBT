...
while ( $count <= $num )
    ...
    set num2 = `cat hmm/output_lab_splice_festival_format/$fn | wc -l`
    set count2 = 2
    echo "#" >> hmm_syllable_er_syl_lab_with_begin_end_phone/$fn
    while ( $count2 <= $num2 )  
        set ln1 = `head -$count2 hmm/output_lab_splice_festival_format/$fn | tail -1`
        ...
        @ count3--
        set phoneme2 = `echo $transcription | cut -d " " -f$count3` 
        echo $ln1 $phoneme1 $phoneme2 >> hmm_syllable_er_syl_lab_with_begin_end_phone/$fn
        @ count2++
        end
    @ count++
end
